...
const DEFAULT_CSV = path.join(__dirname, '..', 'excel_files', 'Logistics_sheet.csv');

const FIELD_MAP = {
  'title': 'unique_id',
  'sample id': 'project_id',
  'date of sample collected': 'sample_collection_date',
  'shipment date': 'sample_shipped_date',
  'delivery date': 'sample_delivery_date',
  'sales person': 'sales_responsible_person',
  'sender name': 'third_party_name',
  'sender organisation': 'organisation_hospital',
  'sender city': 'delivery_up_to',
  'sender contact': 'third_party_phone',
  'receiver address': 'patient_client_address',
  'tracking id': 'tracking_id',
  'courier company': 'courier_company',
  'alert to labprocess team': 'alert_to_labprocess_team',
  'created': 'created_at',
...
async function main(){
  ...
  const [colsRes] = await pool.query(`SELECT COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?`, [process.env.DB_NAME || 'lead_lims2','sample_tracking']);
  ...
  for(const row of rows){ processed++; const out={};
    for(const [csvHeader, dbCol] of Object.entries(csvToDb)){
      ...
      if(colInfo.dataType==='time'){ out[dbCol]=parseTimeFromValue(val); continue; }
      // For alert_to_labprocess_team default to 1 when value is missing/empty in CSV
      if (dbCol === 'alert_to_labprocess_team'){
    ...
    if(!out.unique_id){ out.unique_id=`LOG${Date.now()}${Math.floor(Math.random()*9999)}`; }

...
const DEFAULT_CSV = path.join(__dirname, '..', 'excel_files', 'Lab_process_clinical_samples.csv');
const TABLE_NAME = 'labprocess_clinical_sheet';

// empty FIELD_MAP: we'll attempt to auto-map CSV headers to DB columns by normalized names
const FIELD_MAP = {};
...
async function main(){
  ...
  // Database credentials embedded per request. Replace before committing if necessary.
  const DB_CONFIG = {
    host: 'localhost',
    user: 'remote_user',
    password: 'Prolab#05',
  ...
  for(const h of csvHeaders){ const nh=normalizeHeader(h); if(FIELD_MAP[nh]){ const dbCol=FIELD_MAP[nh]; if(dbColumns[dbCol]) csvToDb[h]=dbCol; continue; } // try auto-match by exact column name
    for(const dbCol of Object.keys(dbColumns)){ if(dbCol && normalizeHeader(dbCol)===nh){ csvToDb[h]=dbCol; break; } }
  }
  ...
  for(const row of rows){ processed++; const out={};
    ...
    if(!out.unique_id){ out.unique_id=`LPCL${Date.now()}${Math.floor(Math.random()*9999)}`; }
